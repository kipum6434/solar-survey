...
## Customer Creation Date Column
- [x] เพิ่มคอลัมน์ “วันที่เพิ่มรายชื่อ” ในตารางหน้าลูกค้าทั้งหมด
- [x] ย้ายคอลัมน์ให้อยู่ถัดจากเบอร์โทรและจัดรูปแบบวัน/เดือน/ปี พ.ศ.
- [x] เพิ่มวันที่ในมุมมองการ์ดเพื่อรองรับหน้าจอมือถือ
- [x] ทดสอบ formatCustomerCreatedDate 2 กรณี และตรวจสอบ TypeScript

## Customer Creation Date Range Filter
- [x] เพิ่มพารามิเตอร์กรองวันเริ่มต้นและวันสิ้นสุดใน customer.list
- [x] เพิ่มตัวเลือกช่วงวันที่เพิ่มรายชื่อในหน้าลูกค้าทั้งหมด
- [x] แสดงสถานะช่วงวันที่ที่กำลังกรองและปุ่มล้างตัวกรอง
- [x] ทดสอบขอบเขตวันตามเวลาไทย 3 กรณีและตรวจสอบ TypeScript

## Installation Photos Availability Incident — Reported 14 Aug 2569+
- [x] ตรวจสอบจำนวนรายการรูปติดตั้งในฐานข้อมูลก่อนและหลัง 14 ส.ค. 2569
- [x] ตรวจสอบการอ้างอิงไฟล์ S3 และความสามารถในการเข้าถึงไฟล์
- [x] ตรวจสอบ query, filter และการลบรูปที่อาจทำให้รูปไม่แสดง
- [x] สำรองรายการอ้างอิงปัจจุบันและกู้คืน metadata ของรูปที่ยังอยู่ใน S3
- [x] เพิ่ม audit log ให้การลบรูปผ่านลิงก์สาธารณะเพื่อระบุผู้และเวลาที่ลบในอนาคต

ผลตรวจสอบและกู้คืน: ก่อนกู้คืนมี 223 รายการหลัง 14 ส.ค. ของ 1 งานใน DB แต่ S3 มี 5,741 ไฟล์ใน 58 งาน. สำรอง `installation_photos` จำนวน 19,235 รายการไว้ใน `installation_photos_backup_before_recovery_20260825` แล้วกู้คืน metadata ได้ 5,168 รายการให้ 53 งาน; หลังการกู้คืน DB มี 5,391 รายการหลัง 14 ส.ค. ใน 53 งาน และเปิดไฟล์ตัวอย่างจาก S3 ได้จริง. S3 อีก 5 โฟลเดอร์ (งาน 11430001, 11580001, 11670001, 11790001, 11820001) ไม่มี survey record จึงยังไม่สร้าง metadata เพื่อป้องกันการผูกข้อมูลผิดงาน.

การป้องกันเพิ่มเติม: publicDelete จะบันทึก activity log พร้อม surveyId, shareLinkId, photoId และ fileKey ทุกครั้ง โดยไม่บันทึก token ลิงก์สาธารณะ. Unit test ผ่าน 1 รายการและ TypeScript ผ่าน.

## Share Link Photo Incident — 6UC6hv6mzUAW8EBbH84Qaa92NiF7Ujap
- [x] ระบุ survey ที่เชื่อมกับ share link และตรวจสถานะรูปติดตั้งในฐานข้อมูล
- [x] ตรวจไฟล์รูปติดตั้งของงานเดียวกันใน S3 โดยไม่แก้ไขข้อมูล
- [x] ยืนยันหน้า share link แสดงรูปติดตั้งที่กู้คืนแล้วในหมวดอื่นๆ

ผลตรวจ: share link เชื่อมกับงาน 10890001 ของคุณณิชนันทน์ ยุ่นสมาน; หน้าสาธารณะแสดงรูปสำรวจ 24 รูป และในส่วนส่งมอบงานมีรูปติดตั้ง 112 รูปอยู่ในหมวด “อื่นๆ” ซึ่งเปิดจาก S3 ได้จริง. 12 หมวดตรวจรับจึงแสดง 0/12 เพราะ metadata ที่กู้คืนก่อนหน้านี้จำเป็นต้องกำหนด category เป็น `other` เนื่องจาก S3 key ไม่มีข้อมูลหมวดเดิม. ไม่พบว่ารูปไฟล์หายจากลิงก์นี้

## Reclassify Recovered Installation Photos
- [x] สำรอง category ปัจจุบันของรูปกู้คืนทั้งหมดก่อนปรับแก้
- [x] จัดทำเกณฑ์ AI สำหรับแยกหมวดภาพติดตั้ง พร้อมเก็บรูปที่ไม่มั่นใจในหมวดอื่นๆ
- [x] จัดหมวดรูปที่มั่นใจสูงให้ถูกต้องในทุกงานที่ได้รับผลกระทบ
- [x] ตรวจจำนวนรูปและความครอบคลุมของ checklist หลังจัดหมวด

ผลดำเนินการ: สำรองรูปหมวดอื่นๆ 5,352 รายการเป็นไฟล์เข้ารหัสใน S3 (`private-backups/installation-photo-categories-before-ai-1787663508053_73b6183c.bin`) แล้วใช้ Vision AI จัดหมวด โดยย้ายเฉพาะผลที่มั่นใจสูง 3,244 รูปไปยังหมวดตรวจรับที่สอดคล้องกัน และคง 2,108 รูปที่ไม่มั่นใจหรือไม่เข้าหมวดไว้ใน `other`. ผลตรวจงาน 10890001 บน share link แสดงรูปในหมวดที่ถูกต้องและ checklist ครบ 12/12 หมวด (100%).
