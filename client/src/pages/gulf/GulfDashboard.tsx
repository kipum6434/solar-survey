...
export default function GulfDashboard() {
  ...
  const { data: stats, isLoading } = trpc.dashboard.gulfStats.useQuery();

  const statCards = [
    { label: "ลูกค้า Gulf", value: stats?.totalCustomers ?? 0, icon: Users, color: "from-blue-500 to-blue-600", link: "/gulf/customers" },
    { label: "งานสำรวจ", value: stats?.totalSurveys ?? 0, icon: ClipboardList, color: "from-amber-500 to-amber-600", link: "/gulf/surveys" },
    { label: "รอสำรวจ", value: stats?.pendingSurveys ?? 0, icon: Clock, color: "from-orange-500 to-orange-600", link: "/gulf/surveys" },
    { label: "สำรวจเสร็จ", value: stats?.completedSurveys ?? 0, icon: CheckCircle2, color: "from-emerald-500 to-emerald-600", link: "/gulf/surveys" },
    { label: "ปิดการขาย", value: stats?.wonDeals ?? 0, icon: Trophy, color: "from-green-500 to-green-600", link: "/gulf/surveys" },
    { label: "รอ Follow-up", value: stats?.pendingFollowUps ?? 0, icon: PhoneCall, color: "from-purple-500 to-purple-600", link: "/gulf/follow-ups" },
  ];

  const installCards = [
    { label: "งานติดตั้งทั้งหมด", value: stats?.totalInstallations ?? 0, icon: Wrench, color: "from-cyan-500 to-cyan-600", link: "/gulf/installations" },
    { label: "กำลังติดตั้ง", value: stats?.inProgressInstallations ?? 0, icon: HardHat, color: "from-indigo-500 to-indigo-600", link: "/gulf/installations" },
    { label: "ติดตั้งเสร็จ", value: stats?.completedInstallations ?? 0, icon: PackageCheck, color: "from-teal-500 to-teal-600", link: "/gulf/installations" },
  ];
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">ภาพรวมลูกค้าและงาน Gulf</p>
          </div>
          <Button onClick={() => setLocation("/gulf/customers")} size="sm" className="gap-2">
            <Users className="h-4 w-4" /> ดูลูกค้า Gulf
          </Button>
        </div>
        ...
        <Card>
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-base">งานสำรวจ Gulf ล่าสุด</CardTitle>
              <Button variant="ghost" size="sm" className="gap-1 text-xs" onClick={() => setLocation("/gulf/surveys")}>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-6">ยังไม่มีงานสำรวจ Gulf</p>
            ) : (
                ...
                {stats.recentSurveys.map((s: any) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => setLocation(`/gulf/surveys`)}
                    >
